(function () {
    ...
    const getCssVar = (name) => {
        const val = getComputedStyle(document.body || document.documentElement).getPropertyValue(name).trim();
        return val || name;
    };
    ...
    window.renderSparkline = function (card) {
        ...
        items.forEach((item) => {
            ...
            if (!isNaN(d.getTime())) rawData.push([d.getTime(), val]);
        });
        ...
        const thresholds = {
            nmin: parseNum(card.dataset.nmin),
            nmax: parseNum(card.dataset.nmax),
            cmin: parseNum(card.dataset.cmin),
            cmax: parseNum(card.dataset.cmax)
        };
        const view = {
            min: parseNum(card.dataset.dmin),
            max: parseNum(card.dataset.dmax)
        };
        ...
        let { bMin, bMax } = computeAxisBounds(view.min, view.max, rawData);
        let options;
        ...
        if (type === 'gauge') {
            ...
            if (Number.isFinite(thresholds.nmin) && Number.isFinite(thresholds.nmax) && bMax > bMin) {
                let nminP = Math.max(0, Math.min(1, (thresholds.nmin - bMin) / (bMax - bMin)));
                let nmaxP = Math.max(0, Math.min(1, (thresholds.nmax - bMin) / (bMax - bMin)));
                const cYellow = resolveColor('var(--chart-band-yellow)') || '#f59e0b';
                const cGreen = resolveColor('var(--chart-band-green)') || '#10b981';
                const cRed = resolveColor('var(--chart-band-red)') || '#ef4444';
            ...
            options = {
                ...
                series: [{
                    ...
                    radius: '90%',
                    center: ['50%', '60%'],
                    axisLine: { lineStyle: { width: 10, color: gaugeColors.length > 0 ? gaugeColors : [[1, chartColor]] } },
            ...
            options = {
                grid: { top: 10, bottom: 0, left: 0, right: 15, containLabel: true },
                tooltip: {
                    trigger: 'axis',
    ...
    cards.forEach((card) => { window.renderSparkline(card); });
    ...
    document.addEventListener('change', function (e) {
        if (!e.target.classList.contains('card-interval-select')) return;
        ...
        const formData = new FormData();
        formData.append('parameter_id', card.dataset.slug);
    ...
    window.addEventListener('DashMedMetricsUpdate', function (event) {
        ...
        try {
            ...
            metrics.forEach(metric => {
                ...
                if (!dataList || !metric.time_iso || metric.value === '') return;
